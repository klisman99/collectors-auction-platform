...
class OperationalAccountHttpIntegrationTests {
  ...
  private static final Pattern ACTIVATION_TOKEN =
      Pattern.compile("operationalActivationToken=([^\\s]+)");
  ...
  void clearMessages() {
    ...
    jdbcTemplate.update(
        "DELETE FROM operational_accounts WHERE normalized_email <> ?", "bootstrap-29@example.com");
  ...
  private <S extends Session> void assertSessionPersistence(
  ...
  void administratorInvitesActivatesAndAuthenticatesADedicatedModerator() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/admin/operational-accounts")).andExpect(status().isUnauthorized());

    MvcResult administrator =
        signIn(
                csrf,
                "bootstrap-29@example.com",
    ...
    MvcResult invitationResult =
        mockMvc
            .perform(
                post("/api/v1/admin/operational-accounts")
    ...
    assertThat(invitation.getSubject())
        .isEqualTo("You are invited to Collectors Auction Platform operations");
    ...
    mockMvc
        .perform(
            post("/api/v1/auth/activate-operational-account")
                .cookie(csrf.cookie())
                .header("X-XSRF-TOKEN", csrf.token())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
    ...
    mockMvc
        .perform(
            get("/api/v1/admin/audit-records").cookie(csrf.cookie(), sessionCookie(administrator)))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath(
                    "$[?(@.targetId == '%s' && @.action == 'OPERATIONAL_ACCOUNT_INVITED')].metadata"
        ...
        .andExpect(
            jsonPath(
                    "$[?(@.targetId == '%s' && @.action == 'OPERATIONAL_ACCOUNT_ACTIVATED')]"
                        .formatted(invitedAccountId))
                .isNotEmpty());
  }
    ...
    mockMvc
        .perform(
            post("/api/v1/auth/register")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"email\":\"%s\",\"publicHandle\":\"regular_first_%s\",\"password\":\"regular account password\"}"
  ...
  @Test
  void administratorCanRevokePendingInvitationsAndExpiredInvitationsCannotActivate()
      throws Exception {
    Csrf csrf = csrf();
    MvcResult administrator =
        signIn(
    ...
    mockMvc
        .perform(
            post("/api/v1/admin/operational-accounts/" + pendingAccountId + "/deactivate")
                .cookie(csrf.cookie(), sessionCookie(administrator))
                .header("X-XSRF-TOKEN", csrf.token())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
    ...
    jdbcTemplate.update(
        "UPDATE operational_account_invitations SET expires_at = ? WHERE operational_account_id = ?",
        Timestamp.from(Instant.now().minusSeconds(1)),
        UUID.fromString(expiredAccountId));
  ...
  @Test
  void deactivationRevokesSessionsPreservesAttributionAndProtectsTheLastAdministrator()
      throws Exception {
    Csrf csrf = csrf();
    MvcResult administrator =
        signIn(
    ...
    String administratorId =
        objectMapper
            .readTree(administrator.getResponse().getContentAsString())
            .get("accountId")
            .asText();

    String accountId =
        inviteAndActivate(csrf, administrator, "administrator-29@example.com", "ADMINISTRATOR");
    ...
    mockMvc
        .perform(
            post("/api/v1/admin/operational-accounts/" + accountId + "/deactivate")
                .cookie(csrf.cookie(), sessionCookie(administrator))
                .header("X-XSRF-TOKEN", csrf.token())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
    ...
    mockMvc
        ...
        .andExpect(status().isUnauthorized());
    mockMvc
        .perform(get("/api/v1/auth/session").cookie(sessionCookie(secondTargetSession)))
        .andExpect(status().isUnauthorized());
    signIn(
            csrf,
            "administrator-29@example.com",
    ...
    mockMvc
        .perform(
            get("/api/v1/admin/operational-accounts")
                .cookie(csrf.cookie(), sessionCookie(administrator)))
        .andExpect(status().isOk())
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                    "$[?(@.targetId == '%s' && @.action == 'OPERATIONAL_ACCOUNT_DEACTIVATED')].actorId"
                        .formatted(accountId))
                .value(org.hamcrest.Matchers.hasItem(administratorId.toString())));

    mockMvc
        .perform(
            post("/api/v1/admin/operational-accounts/" + administratorId + "/deactivate")
                .cookie(csrf.cookie(), sessionCookie(administrator))
                .header("X-XSRF-TOKEN", csrf.token())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  @Test
  void operationalAccountsCannotUseTradingCommandsAndModeratorsCannotManageAccounts()
      throws Exception {
    Csrf csrf = csrf();
    MvcResult administrator =
        signIn(
    ...
    MvcResult moderator =
        signIn(csrf, "moderator-29b@example.com", "moderator activation password", "198.51.100.135")
            .andExpect(status().isOk())
            .andReturn();

    mockMvc
        .perform(get("/api/v1/admin/operational-accounts").cookie(sessionCookie(moderator)))
  ...
  void concurrentAdministratorDeactivationsRetainOneActiveAdministrator() throws Exception {
    ...
    String firstAdministratorId =
        inviteAndActivate(csrf, administrator, "administrator-29a@example.com", "ADMINISTRATOR");
    String secondAdministratorId =
        inviteAndActivate(csrf, administrator, "administrator-29b@example.com", "ADMINISTRATOR");
          ...
          executor.submit(
              () ->
                  deactivate(
                      ...
                      "Remove second administrator"));
      assertThat(java.util.List.of(first.get(), second.get())).containsExactlyInAnyOrder(204, 204);
    }

    mockMvc
        .perform(
            get("/api/v1/admin/operational-accounts").cookie(csrf.cookie(), administratorSession))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$[?(@.role == 'ADMINISTRATOR' && @.status == 'ACTIVE')]")
  ...
  void concurrentActivationAndRevocationHaveOneDeterministicOutcome() throws Exception {
    ...
    try (var executor = Executors.newFixedThreadPool(2)) {
      ...
      Future<Integer> deactivation =
          executor.submit(
              () -> deactivate(csrf, administratorSession, accountId, "Revoke raced invitation"));
  ...
  @Test
  void consumesAnOperationalInvitationExactlyOnceWhenActivationIsRetriedConcurrently()
      throws Exception {
    Csrf csrf = csrf();
    MvcResult administrator =
        signIn(
  ...
  private String inviteAndActivate(Csrf csrf, MvcResult administrator, String email, String role)
      throws Exception {
  ...
  private String invite(Csrf csrf, MvcResult administrator, String email, String role)
      throws Exception {
  ...
  private org.springframework.test.web.servlet.ResultActions activateWithStatus(
      Csrf csrf, String token) throws Exception {
  ...
  private int deactivate(
      Csrf csrf, Cookie administratorSession, String accountId, String publicReason)
      throws Exception {
    return mockMvc
        .perform(
            post("/api/v1/admin/operational-accounts/" + accountId + "/deactivate")
