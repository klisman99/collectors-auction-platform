...
class InitialAdministratorService {
    ...
    void ensureConfiguredAdministrator(String email, String password) {
        ...
        OperationalAccount account = OperationalAccount.initialAdministrator(
                UUID.randomUUID(),
                normalizedEmail,
                passwordEncoder.encode(password),
                now);
        operationalAccounts.saveAndFlush(account);
        events.publishEvent(new InitialAdministratorCreated(account.id(), account.normalizedEmail(), now));
    }
}
