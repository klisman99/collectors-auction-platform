...
@RestController
@RequestMapping(
    path = "/api/v1/admin/operational-accounts",
...
class OperationalAccountController {
  ...
  @Operation(
      operationId = "inviteOperationalAccount",
      summary = "Invite a dedicated moderator or administrator")
  OperationalAccountResponse invite(
      @Valid @RequestBody InviteOperationalAccountRequest request, Authentication authentication) {
    OperationalAccountService.InvitationResult result =
        service.invite(
  ...
  @Operation(
      ...
      summary = "List operational accounts for administrators")
  ...
  @Operation(
      operationId = "deactivateOperationalAccount",
      summary = "Permanently deactivate an operational account")
  void deactivate(
  ...
  record InviteOperationalAccountRequest(
    ...
    InviteOperationalAccountRequest {
      ...
      reasonCategory =
          reasonCategory == null ? null : reasonCategory.trim().toUpperCase(java.util.Locale.ROOT);
  ...
  record OperationalAccountResponse(UUID id, String email, String role, String status) {
    ...
    static OperationalAccountResponse from(OperationalAccountService.InvitationResult result) {
      return new OperationalAccountResponse(
          result.id(), result.email(), result.role(), result.status());
    }
  }
}
