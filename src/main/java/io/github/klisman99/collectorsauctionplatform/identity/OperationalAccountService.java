...
class OperationalAccountService {
  ...
  InvitationResult invite(
    ...
    OperationalAccountInvitation invitation =
        new OperationalAccountInvitation(
            UUID.randomUUID(),
            account.id(),
  ...
  void deactivate(
    ...
    if (account.isActiveAdministrator()
        && operationalAccounts.countByRoleAndStatus(
    ...
    invitations
        .findUnusedByOperationalAccountIdForUpdate(account.id())
        .forEach(invitation -> invitation.markUsed(now));
    sessions.revokeAll(account.id());
    events.publishEvent(
        new OperationalAccountDeactivated(
            account.id(),
            actorId,
            account.role().name(),
            category.name(),
  ...
  record InvitationResult(UUID id, String email, String role, String status) {
    ...
    static InvitationResult from(OperationalAccount account) {
      return new InvitationResult(
          account.id(), account.normalizedEmail(), account.role().name(), account.status().name());
    }
  }
