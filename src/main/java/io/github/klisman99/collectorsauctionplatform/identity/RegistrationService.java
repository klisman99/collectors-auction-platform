...
class RegistrationService {
    ...
    RegistrationResult register(String email, String handle, String password) {
        ...
        RegularAccount account = RegularAccount.register(
                UUID.randomUUID(),
                normalizedEmail,
                publicHandle,
        ...
        events.publishEvent(new RegularAccountRegistered(
                account.id(),
    ...
    VerificationResult verify(String rawToken) {
        Instant now = Instant.now(clock);
        EmailVerificationToken token = verificationTokens.findByTokenDigest(tokenGenerator.digest(rawToken))
    ...
    private IdentityApiException resolveRegistrationConflict(DataIntegrityViolationException exception) {
